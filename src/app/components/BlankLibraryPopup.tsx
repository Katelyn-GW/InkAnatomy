import svgPaths from "../../imports/svg-arwyrzzhtx";
...
interface BlankLibraryPopupProps {
  imageUrl?: string;
  name?: string;
  description?: string;
  isEditing?: boolean;
...
export default function BlankLibraryPopup({
  imageUrl,
  name = '',
  description = '',
  isEditing = false,
  ...
  return (
    <div className="bg-[#427f6e] relative size-full" data-name="Photo library- Image selected blank">
      ...
      {/* Rabbit image - keep it visible without blur */}
      ...
      {uploadedImages.slice(0, 11).map((image, index) => {
        ...
        return (
          <div key={image.id}>
            ...
            <div
              className="absolute h-[127px] rounded-[5px] w-[111px] z-10 pointer-events-none"
      ...
      {/* Image area - show uploaded image or blue placeholder - repositioned and resized */}
      <div className="absolute h-[90px] left-[156px] top-[530px] w-[90px] z-10" data-name="BlankImageArea">
        {imageUrl ? (
          ...
          <div className="absolute inset-0 bg-[#72aea3] rounded-[5px]" />
        )}
      </div>
      {/* Description area - editable or display */}
      <div className="absolute h-[32px] left-[46px] top-[682px] w-[321px] bg-[#d4b894] border border-[#c2a77c] border-solid rounded-[3px] z-10">
        {isEditing ? (
          <input
            type="text"
            value={description}
            ...
            className="w-full h-full bg-transparent px-2 text-[14px] text-black font-['Poppins',sans-serif] outline-none"
            onClick={(e) => e.stopPropagation()}
          />
        ) : (
          <p className="px-2 py-1 text-[14px] text-black font-['Poppins',sans-serif] truncate">
            {description}
          </p>
        )}
      </div>
      ...
      {/* Name input area - above description */}
      <div className="absolute bg-[#8dd7ca] border border-black border-solid h-[36px] left-[135px] rounded-[5px] shadow-[2px_6px_10.4px_2px_rgba(0,0,0,0.25)] top-[620px] w-[135px] z-10">
        ...
        {isEditing ? (
          <input
            ...
            placeholder="Name..."
            className="w-full h-full bg-transparent px-2 text-[14px] text-black font-['Poppins',sans-serif] outline-none text-center relative z-10"
            onClick={(e) => e.stopPropagation()}
          />
        ) : (
          <p className="px-2 py-2 text-[14px] text-black font-['Poppins',sans-serif] text-center truncate relative z-10">
            {name}
          </p>
        )}
      </div>
      
      {/* Eraser icon - always shown with full color for consistency */}
      <div 
        className={`absolute left-[40px] overflow-clip size-[23px] top-[527px] z-20 ${onDelete ? 'cursor-pointer' : 'cursor-not-allowed'}`}
        onClick={(e) => {
          if (onDelete) {
            e.stopPropagation();
            onDelete();
      ...
      <div 
        className={`absolute left-[338px] overflow-clip size-[23px] top-[527px] cursor-pointer z-20 ${isEditing ? 'bg-[#8dd7ca] rounded-full' : ''}`} 
        data-name="Edit"
      >
          ...
          <div className="absolute inset-[-5.19%]">
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 21.2829 21.2829">
              <path 
                d={svgPaths.pcd9b680} 
                id="Icon" 
                stroke={isEditing ? "#028A7B" : "var(--stroke-0, #1E1E1E)"} 
                strokeLinecap="round" 
      ...
      {/* Blank name text - no text for user input */}
